...
aToken = "eyJhbGciOiJSUzI1NiIsImtpZCI6InJISWlodFBYb0VxQ0xUTEhkUkJlVDVDUF9KYWhZbXF2QWs4d241bGR2N00ifQ..WB3l-i9snSFaXyvOXw7khz1Cn245gmUJ8PrH9_OB7T74iP5F1EQRoBhyiKlrG0JWf0yfc6Tc6r7KCrfG8N6E23EhsZGyKz2W_NEXmfM18-HL7V9FVl_sHZAuLm1uFx4Btq3d_koD28obV-HQq9P4DfWsAWq2ndSWsPhPKy1J5O4e485zgH2Kkdl1g4pCbkxkZ9NHhXEp47iIIR9Cu1BdijDvklDnZNYqrBP-Tl1lUIl0wqooY7q1D1X8qL4HnIh46gl0t1zNcuoEWVXGnojMiTpVOXQx6e0KVMk_RTfKHgk-Foxmb3I7GM3NLbgvVd3uvqU7VUC9diU4Umtx8pFQFw"
aConfiguration.host = "https://192.168.58.99:6443"
...
def store(args):
    ...
    if table_exists('images'):
        print("Clean old records of images")
        cur.execute("DROP TABLE images")

    if table_exists('cve'):
        print("Clean old records of cve")
    ...
    if args.namespace:
        ...
        print(f"Looking for pods running on all namespaces")
        ret = v1_core.list_pod_for_all_namespaces(watch=False)

    if ret == [] and args.namespace:
        print(f"Found 0 runnng pods. Dows namespace {args.namespace} exists?")
    ...
    for i in ret.items:
        ...
        for container in pod.spec.containers:
            ...
            print(f"Save record for {container.image}")
    ...
    print("Images scanning completed")

def get_images(args):
    if not table_exists("images"):
        print(f"Please do: ./{sys.argv[0]} store")
        sys.exit(1)
    ...
    rows = cur.fetchall()
    if rows == []:
        print(f"Query ha returned 0 elements. Please do \"{sys.argv[0]} store\"")
    for row in rows:
        retval.append({
            "image": row[0],
            "container": row[1],
...
def vulns(args):
    ...
    print(json.dumps(cve_list, indent = 4))
...
def already_scanned(img):
    ...
    rows = cur.fetchall()
    if rows != []:
        print(f"Image {img} already checked")
        return True
...
if __name__ == '__main__':
    if not tool_exists("trivy"):
        print("trivy is not installed on this system, please install it")
        sys.exit(1)
